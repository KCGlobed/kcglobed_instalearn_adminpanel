...
const schema = yup.object().shape({
    title_l: yup.string().required("Name is required"),
    title_2: yup.string().required("Qualification is required"),
    'title-3': yup.string().required("Company name is required"),
    experience: yup.string().required("Experience is required"),
    'linkedin-url': yup.string()
    ...
    company_image_l: yup.mixed().nullable().default(null),
});
...
const InstructorPublicProfileForm = ({ instructorId }: { instructorId: string | number }) => {
    ...
    const [imagePreviews, setImagePreviews] = useState<{
        image?: string;
        company_image_l?: string;
    }>({});

    // Image error states
    const [imageError, setImageError] = useState<string | null>(null);
    const [companyImageError, setCompanyImageError] = useState<string | null>(null);

    // Cropper State
    const [rawImageSrc, setRawImageSrc] = useState<string>('');
    const [isCropperOpen, setIsCropperOpen] = useState(false);
    const [originalImageFormat, setOriginalImageFormat] = useState<'image/jpeg' | 'image/png'>('image/jpeg');
    const [activeCropField, setActiveCropField] = useState<'image' | 'company_image_l' | null>(null);
    ...
    const {
        ...
        defaultValues: {
            ...
            'linkedin-url': '',
            image: null,
            company_image_l: null,
        }
    });

    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: 'image' | 'company_image_l') => {
    ...
    const handleCropComplete = (result: CropResult) => {
        if (activeCropField) {
            ...
            if (activeCropField === 'company_image_l') setCompanyImageError(null);
        }
        setIsCropperOpen(false);
        setActiveCropField(null);
    };

    const handleRemoveFile = (fieldName: 'image' | 'company_image_l') => {
    ...
    const onSubmit = async (data: FormValues) => {
        ...
        if (!imagePreviews.company_image_l && !data.company_image_l) {
            setCompanyImageError("Company image is required");
            hasImageError = true;
        } else {
        ...
        try {
            ...
            formData.append('title_l', (data.title_l || '').trim());
            formData.append('title_2', (data.title_2 || '').trim());
            formData.append('title-3', (data['title-3'] || '').trim());
            formData.append('experience', (data.experience || '').trim());
            formData.append('linkedin-url', (data['linkedin-url'] || '').trim());

            if (data.image instanceof File) {
                formData.append('image', data.image);
            }
            if (data.company_image_l instanceof File) {
                formData.append('company_image_l', data.company_image_l);
            }
            ...
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 flex gap-3 items-center">
                    ...
                    <div className="flex-1">
                        ...
                        <p className="text-[11px] text-indigo-600/80 mt-0.5">
                            Configure public titles, professional experience, LinkedIn link, and company logo.
                        </p>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3.5">
                    ...
                    <div>
                        ...
                        {errors.title_l && (
                            <p className="flex items-center gap-1 mt-1 text-xs font-medium text-red-500">
                                <AlertCircle size={12} /> {errors.title_l.message}
                            </p>
                        )}
                    </div>
                    ...
                    <div>
                        ...
                        {errors['title-3'] && (
                            <p className="flex items-center gap-1 mt-1 text-xs font-medium text-red-500">
                                <AlertCircle size={12} /> {errors['title-3'].message}
                            </p>
                        )}
                    </div>
                    ...
                    <div className="md:col-span-2">
                        ...
                        {errors['linkedin-url'] && (
                            <p className="flex items-center gap-1.5 mt-1.5 text-xs font-medium text-red-500">
                                <AlertCircle size={13} /> {errors['linkedin-url']?.message}
                            </p>
                        )}
                    </div>
                </div>
                ...
                <div className="space-y-3 pt-3 border-t border-gray-100">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        {/* Company Image L */}
                        <div className="flex flex-col gap-1">
                            <label className="text-xs font-semibold text-gray-600">Company Image <span className="text-red-500">*</span></label>
                            <div className={`relative border border-dashed hover:border-indigo-400 transition-colors rounded-xl p-1 flex items-center justify-center bg-gray-50/50 min-h-[56px] h-[56px] ${
                                companyImageError ? 'border-red-400 bg-red-50/10' : 'border-gray-200'
                            }`}>
                                {imagePreviews.company_image_l ? (
                                    <div className="relative flex items-center gap-2.5 w-full h-full px-2">
                                        <div className="w-9 h-9 rounded-lg overflow-hidden border border-gray-200 bg-white flex items-center justify-center p-0.5 flex-shrink-0">
                                            <img src={imagePreviews.company_image_l} alt="Logo" className="max-w-full max-h-full object-contain" />
                                        </div>
                                        ...
                                        <button
                                            ...
                                            onClick={(e) => {
                                                e.stopPropagation();
                                                handleRemoveFile('company_image_l');
                                            }}
                                            className="absolute top-1 right-1 bg-red-50 text-red-500 hover:bg-red-100 p-1 rounded-full transition-colors flex items-center justify-center shadow-sm flex-shrink-0"
                                            title="Remove logo"
                                        >
                                    ...
                                    <label className="cursor-pointer flex items-center justify-center gap-2 w-full h-full">
                                        ...
                                        <input
                                            type="file"
                                            accept="image/*"
                                            onChange={(e) => handleFileChange(e, 'company_image_l')}
                                            className="hidden"
                                        />
                                    </label>
                                )}
                            ...
                            {companyImageError && (
                                <p className="flex items-center gap-1 mt-1 text-xs font-medium text-red-500">
                                    <AlertCircle size={12} /> {companyImageError}
                                </p>
                            )}
                        </div>
