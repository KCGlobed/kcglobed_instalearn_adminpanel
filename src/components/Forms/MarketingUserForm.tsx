...
import { createMarketingUser, updateMarketingUser, getMarketingUsers } from '../../store/slices/marketingUserSlice';
...
type Props = {
    MarketingUserData?: any;
};

const MarketingUserForm = ({ MarketingUserData }: Props) => {
    ...
    useEffect(() => {
        if (MarketingUserData) {
            const fbCountryIso = getCountryIso(MarketingUserData.country);
            const fbStateIso = getStateIso(fbCountryIso, MarketingUserData.state);
            reset({
                first_name: MarketingUserData.first_name || '',
                last_name: MarketingUserData.last_name || '',
                email: MarketingUserData.email || '',
                address: MarketingUserData.address || '',
                city: MarketingUserData.city || '',
                state: fbStateIso,
                country: fbCountryIso,
                pincode: MarketingUserData.pincode || '',
                dob: MarketingUserData.dob ? MarketingUserData.dob.split('T')[0] : '',
            });
        }
    }, [MarketingUserData, reset]);

    // Submit handler
    const onSubmit = async (data: MarketingUserFormValues) => {
        ...
        try {
            ...
            if (MarketingUserData?.id) {
                const actionResult = await dispatch(updateMarketingUser({ id: MarketingUserData.id, marketingUserData: formdata }));
                if (updateMarketingUser.fulfilled.match(actionResult)) {
                    toast.success("Marketing User updated successfully");
                }
            } else {
                await dispatch(createMarketingUser(formdata)).unwrap();
                toast.success("Marketing User created successfully");
            }

            reset();
            hideModal();
        } catch (err: any) {
            console.error('Marketing User creation failed:', err);
            toast.error(err || "Failed to create Marketing User");
        } finally {
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                ...
                                {MarketingUserData ? 'Updating Marketing User...' : 'Creating Marketing User...'}
                            </>
                        ) : MarketingUserData ? 'Update Marketing User' : 'Create Marketing User'}
                    </button>
                </div>
            </form>
        </div>
