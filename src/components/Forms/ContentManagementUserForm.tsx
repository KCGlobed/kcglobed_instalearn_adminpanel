...
import { createContentManagementUser, updateContentManagementUser } from '../../store/slices/contentManagementSlice';
import { Country, State, City } from 'country-state-city';
import Select from 'react-select'
import toast from 'react-hot-toast';

type ContentManagementUserFormValues = {
    first_name: string;
    last_name: string;
    email: string;
    address: string;
...
type Props = {
    ContentManagementUserData?: any;
};

const ContentManagementUserForm = ({ ContentManagementUserData }: Props) => {
    ...
    const {
        ...
        control,
        setValue
    } = useForm<ContentManagementUserFormValues>({
        defaultValues: {
            first_name: '',
            last_name: '',
            email: '',
    ...
    useEffect(() => {
        if (ContentManagementUserData) {
            const fbCountryIso = getCountryIso(ContentManagementUserData.country);
            const fbStateIso = getStateIso(fbCountryIso, ContentManagementUserData.state);
            reset({
                first_name: ContentManagementUserData.first_name || '',
                last_name: ContentManagementUserData.last_name || '',
                email: ContentManagementUserData.email || '',
                address: ContentManagementUserData.address || '',
                city: ContentManagementUserData.city || '',
                state: fbStateIso,
                country: fbCountryIso,
                pincode: ContentManagementUserData.pincode || '',
                dob: ContentManagementUserData.dob ? ContentManagementUserData.dob.split('T')[0] : '',
            });
        }
    }, [ContentManagementUserData, reset]);

    // Submit handler
    const onSubmit = async (data: ContentManagementUserFormValues) => {
        ...
        try {
            ...
            if (ContentManagementUserData?.id) {
                const actionResult = await dispatch(updateContentManagementUser({ id: ContentManagementUserData.id, userData: formdata }));
                if (updateContentManagementUser.fulfilled.match(actionResult)) {
                    toast.success("Content Management User updated successfully");
                }
            } else {
                await dispatch(createContentManagementUser(formdata)).unwrap();
                toast.success("Content Management User created successfully");
            }

            reset();
            hideModal();
        } catch (err: any) {
            console.error('Content Management User creation failed:', err);
            toast.error(err || "Failed to create Content Management User");
        } finally {
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                ...
                                {ContentManagementUserData ? 'Updating Content Management User...' : 'Creating Content Management User...'}
                            </>
                        ) : ContentManagementUserData ? 'Update Content Management User' : 'Create Content Management User'}
                    </button>
                </div>
            </form>
        </div>
...
export default ContentManagementUserForm;
