...
import { createFinanceUser, updateFinanceUser, getFinanceUsers } from '../../store/slices/financeUserSlice';
import { Country, State, City } from 'country-state-city';
import Select from 'react-select'
import toast from 'react-hot-toast';

type FinanceUserFormValues = {
    first_name: string;
    last_name: string;
    email: string;
    address: string;
...
const FinanceUserForm = ({ FinanceUserData }: Props) => {
    ...
    useEffect(() => {
        if (FinanceUserData) {
            const fbCountryIso = getCountryIso(FinanceUserData.country);
            const fbStateIso = getStateIso(fbCountryIso, FinanceUserData.state);
            reset({
                first_name: FinanceUserData.first_name || '',
                last_name: FinanceUserData.last_name || '',
                email: FinanceUserData.email || '',
                address: FinanceUserData.address || '',
                city: FinanceUserData.city || '',
                state: fbStateIso,
                country: fbCountryIso,
                pincode: FinanceUserData.pincode || '',
                dob: FinanceUserData.dob ? FinanceUserData.dob.split('T')[0] : '',
            });
        }
    }, [FinanceUserData, reset]);

    // Submit handler
    const onSubmit = async (data: FinanceUserFormValues) => {
        ...
        try {
            ...
            if (FinanceUserData?.id) {
                const actionResult = await dispatch(updateFinanceUser({ id: FinanceUserData.id, userData: formdata }));
                if (updateFinanceUser.fulfilled.match(actionResult)) {
                    toast.success("Finance User updated successfully");
                }
            } else {
                await dispatch(createFinanceUser(formdata)).unwrap();
            ...
            console.error('Finance User creation failed:', err);
            toast.error(err || "Failed to create Finance User");
        } finally {
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                ...
                                {FinanceUserData ? 'Updating Finance User...' : 'Creating Finance User...'}
                            </>
                        ) : FinanceUserData ? 'Update Finance User' : 'Create Finance User'}
                    </button>
                </div>
            </form>
        </div>
