...
import { createCustomerSupportUser, updateCustomerSupportUser, getCustomerSupportUsers } from '../../store/slices/customerSupportUserSlice';
import { Country, State, City } from 'country-state-city';
import Select from 'react-select'
import toast from 'react-hot-toast';

type CustomerSupportUserFormValues = {
    first_name: string;
    last_name: string;
    email: string;
    address: string;
...
type Props = {
    CustomerSupportUserData?: any;
};

const CustomerSupportUserForm = ({ CustomerSupportUserData }: Props) => {
    ...
    const {
        ...
        control,
        setValue
    } = useForm<CustomerSupportUserFormValues>({
        defaultValues: {
            first_name: '',
            last_name: '',
            email: '',
    ...
    useEffect(() => {
        if (CustomerSupportUserData) {
            const fbCountryIso = getCountryIso(CustomerSupportUserData.country);
            const fbStateIso = getStateIso(fbCountryIso, CustomerSupportUserData.state);
            reset({
                first_name: CustomerSupportUserData.first_name || '',
                last_name: CustomerSupportUserData.last_name || '',
                email: CustomerSupportUserData.email || '',
                address: CustomerSupportUserData.address || '',
                city: CustomerSupportUserData.city || '',
                state: fbStateIso,
                country: fbCountryIso,
                pincode: CustomerSupportUserData.pincode || '',
                dob: CustomerSupportUserData.dob ? CustomerSupportUserData.dob.split('T')[0] : '',
            });
        }
    }, [CustomerSupportUserData, reset]);

    // Submit handler
    const onSubmit = async (data: CustomerSupportUserFormValues) => {
        ...
        try {
            ...
            if (CustomerSupportUserData?.id) {
                const actionResult = await dispatch(updateCustomerSupportUser({ id: CustomerSupportUserData.id, customerSupportUserData: formdata }));
                if (updateCustomerSupportUser.fulfilled.match(actionResult)) {
                    toast.success("Customer Support User updated successfully");
                }
            } else {
                await dispatch(createCustomerSupportUser(formdata)).unwrap();
                toast.success("Customer Support User created successfully");
            }

            reset();
            hideModal();
        } catch (err: any) {
            console.error('Customer Support User creation failed:', err);
            toast.error(err || "Failed to create Customer Support User");
        } finally {
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                ...
                                {CustomerSupportUserData ? 'Updating Marketing User...' : 'Creating Marketing User...'}
                            </>
                        ) : CustomerSupportUserData ? 'Update Marketing User' : 'Create Marketing User'}
                    </button>
                </div>
            </form>
        </div>
...
export default CustomerSupportUserForm;
