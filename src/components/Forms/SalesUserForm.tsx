...
import { createSalesUser, updateSalesUser, getSalesUsers } from '../../store/slices/salesUserSlice';
...
const SalesUserForm = ({ SalesUserData }: Props) => {
    ...
    useEffect(() => {
        if (SalesUserData) {
            const fbCountryIso = getCountryIso(SalesUserData.country);
            const fbStateIso = getStateIso(fbCountryIso, SalesUserData.state);
            reset({
                first_name: SalesUserData.first_name || '',
                last_name: SalesUserData.last_name || '',
                email: SalesUserData.email || '',
                address: SalesUserData.address || '',
                city: SalesUserData.city || '',
                state: fbStateIso,
                country: fbCountryIso,
                pincode: SalesUserData.pincode || '',
                dob: SalesUserData.dob ? SalesUserData.dob.split('T')[0] : '',
            });
        }
    }, [SalesUserData, reset]);
    ...
    const onSubmit = async (data: SalesUserFormValues) => {
        ...
        try {
            ...
            if (SalesUserData?.id) {
                const actionResult = await dispatch(updateSalesUser({ id: SalesUserData.id, salesUserData: formdata }));
                if (updateSalesUser.fulfilled.match(actionResult)) {
                    toast.success("Sales User updated successfully");
                }
            } else {
                await dispatch(createSalesUser(formdata)).unwrap();
            ...
            console.error('Sales User creation failed:', err);
            toast.error(err || "Failed to create Sales User");
        } finally {
                    ...
                    >
                        {isSubmitting ? (
                            <>
                                ...
                                {SalesUserData ? 'Updating Sales User...' : 'Creating Sales User...'}
                            </>
                        ) : SalesUserData ? 'Update Sales User' : 'Create Sales User'}
                    </button>
                </div>
            </form>
        </div>
