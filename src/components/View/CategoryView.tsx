...
import { Info, Image, Type, CheckCircle, XCircle, Calendar, Hash } from 'lucide-react';

interface CategoryViewProps {
    categoryData: any;
}

const CategoryView: React.FC<CategoryViewProps> = ({ categoryData }) => {
    if (!categoryData) {
        return (
            <div className="flex flex-col items-center justify-center p-12 text-center text-red-500 min-h-[300px]">
                <Info size={48} className="mb-4 opacity-20" />
                <p className="font-bold text-lg">Category data not found</p>
            </div>
        );
    }
        ...
        <div className="flex flex-col gap-6 p-2">
            ...
            <div className="flex flex-col md:flex-row items-start md:items-center gap-6 p-6 bg-gray-50 border border-gray-100 rounded-xl shadow-sm">
                ...
                <div className="flex flex-col flex-1">
                    ...
                    {parent && parent.name && (
                        <p className="text-sm font-medium text-indigo-600 mb-2">Subcategory of {parent.name}</p>
                    )}
                    <p className="text-gray-600 text-sm leading-relaxed max-w-2xl">
                        {description || 'No description provided for this category.'}
                    </p>
                </div>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border border-gray-200 shadow-sm flex flex-col gap-4">
                ...
                <div className="flex items-center justify-center p-6 bg-gray-50 rounded-lg border border-dashed border-gray-300">
                    ...
                    >
                        ...
                        <span className="font-bold text-lg" style={{ color: text_code || '#000000' }}>
                            {name}
                        </span>
                    </div>
                </div>
