...
import {
    fetchUniversityStudentReminderListingApi,
    downloadUniversityStudentReminderReportPdfApi,
    downloadUniversityStudentReminderReportExcelApi
} from '../../services/apiServices';
import moment from 'moment';

interface UniversityStudentReminderProps {
    studentId: number;
    courses: any[];
}
...
const UniversityStudentReminder: React.FC<UniversityStudentReminderProps> = ({ studentId, courses }) => {
    ...
    useEffect(() => {
        const loadReminders = async () => {
            ...
            try {
                setLoading(true);
                setError(null);
                const response = await fetchUniversityStudentReminderListingApi(studentId, selectedCourseId);
                if (response?.results) {
    ...
    const handleDownload = async (type: 'pdf' | 'excel') => {
        ...
        try {
            setDownloading(true);
            const apiCall = type === 'pdf' ? downloadUniversityStudentReminderReportPdfApi : downloadUniversityStudentReminderReportExcelApi;
...
export default UniversityStudentReminder;
