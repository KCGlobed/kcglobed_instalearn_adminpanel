...
import {
    fetchUniversityStudentAttemptedQuizApi,
    downloadUniversityStudentQuizReportPdfApi,
    downloadUniversityStudentQuizReportExcelApi
} from '../../services/apiServices';
import moment from 'moment';

interface UniversityStudentQuizReportProps {
    studentId: number;
    courses: any[];
}

const UniversityStudentQuizReport: React.FC<UniversityStudentQuizReportProps> = ({ studentId, courses }) => {
    ...
    useEffect(() => {
        const loadQuizzes = async () => {
            ...
            try {
                setLoading(true);
                setError(null);
                const response = await fetchUniversityStudentAttemptedQuizApi(studentId, selectedCourseId);
                if (response?.results) {
    ...
    const handleDownload = async (type: 'pdf' | 'excel') => {
        ...
        try {
            setDownloading(true);
            const apiCall = type === 'pdf' ? downloadUniversityStudentQuizReportPdfApi : downloadUniversityStudentQuizReportExcelApi;
...
export default UniversityStudentQuizReport;
