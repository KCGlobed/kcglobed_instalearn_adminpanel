...
import { getFeedBackUser } from "../../../store/slices/feedbackSlice";
import {
    coursesDropDownList,
    freeFeedBackdownloadexcel,
    freeFeedBackdownloadpdf,
} from "../../../services/subscriptionService";
import GlassButton from "../../../components/Button/Button";
import { useModal } from "../../../context/ModalContext";
import ViewAttachment from "./ReportModals/ViewAttachment";
import { DateInput, Filter } from "../../../components/ReportComponent/reportFilter";

const FeedBackuserReport = () => {
    const dispatch = useAppDispatch();
    const { data, loading, pagination } = useAppSelector((state) => state.feedbackUser);
    // ------------------- States -------------------
    ...
    // Keep subjects only if still valid
    useEffect(() => {
    ...
    // ------------------- API Fetch -------------------
    // useRef to prevent initial double fire
    ...
    useEffect(() => {
        ...
        dispatch(getFeedBackUser(filters));
    }, [
        currentPage,
        debouncedSearch,
    ...
    useEffect(() => {
        const transformed = data.map((user, index) => ({
            ...
            category: user.user_detail.category,
            subscription_status: user.subscription_status == 2 ? "Active" : "Expired",
            ...
            feedBack_subject_id: user.subject,
    ...
    const columns: ColumnDefinition<any>[] = useMemo(
        () => [
            ...
            { key: "feedBack_subject_id", title: "FeedBack Subject", align: "center" },
            { key: "message", title: "Message", align: "center", width: '300px' },
            { key: "created_at", title: "created At", align: "center" },
            {
                key: "subscription_status", title: "Subscription Status", align: "center", width: '300px',
                render: (_, row) => (
                    <span
            ...
            {
                key: "attachment", title: "Attachment", align: "center", width: '300px',
                render: (_, row) => (
                    <>
                            ...
                            <GlassButton
                                onClick={() => {
                                    showModal({
                                        ...
                                        content: <ViewAttachment row={row} />,
                                        type: 'success',
                                        size: 'xl',
                                    });
                                }}
    ...
    const handleDownloadAttachment = (row: any) => {
        const attachmentUrl = row.attachment;
        if (attachmentUrl) {
            const link = document.createElement("a");
            link.href = attachmentUrl;
            link.target = "_blank";   // <-- next tab
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await freeFeedBackdownloadpdf(filters);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await freeFeedBackdownloadexcel(filters);
            if (res?.report_url) {
                const link = document.createElement("a");
                link.href = res.report_url;
    ...
    return (
        <div className="p-6 space-y-4">
            ...
            {/* Table */}
            <div className="overflow-x-auto bg-white/30 backdrop-blur-md border border-white/20 rounded-2xl shadow-lg p-4">
                <DynamicServerTable
                    data={flattenedData}
                    columns={columns}
...
export default FeedBackuserReport;
