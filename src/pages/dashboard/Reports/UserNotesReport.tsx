...
import {userNoteIndividualPdfApi, UserNotesReportExcelApi, UserNotesReportPdfApi} from '../../../services/ReportService';
...
import { getUserNotesReport } from '../../../store/slices/reportNoteSlice';
import GlassButton from '../../../components/Button/Button';
import ViewNotesReport from './ReportModals/ViewNotesReport';

const UserNotesReport = () => {
    const dispatch = useAppDispatch();
    const { showModal } = useModal();
    const { data, loading, pagination } = useAppSelector((state) => state.reportUserNote);
    ...
    useEffect(() => {
        ...
        dispatch(getUserNotesReport(filters));
    }, [
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'notes_count', title: 'Notes Count', align: 'center' },
        {
                        key: 'actions',
                        title: 'Actions',
                        align: 'center',
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserNotesReportPdfApi(filters);
    ...
    const handleIndividualReportPdf = async (data:any) => {
        try {
            const res = await userNoteIndividualPdfApi(data.user_id,data.subject_id); ;
            const fileUrl = res?.data?.report_url;
            if (fileUrl) window.open(fileUrl, "_blank");
        } finally {
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserNotesReportExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="backdrop-blur-md bg-white/30 border border-white/20 rounded-2xl p-4 shadow-lg">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-gray-800">User Notes Reports</h2>
                    <button
...
export default UserNotesReport;
