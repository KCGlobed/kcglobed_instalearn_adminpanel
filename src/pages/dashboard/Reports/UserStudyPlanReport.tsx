...
import {  UserStudyPlanExcelApi, userStudyPlanPdfApi, UserStudyPlanPdfApi } from '../../../services/ReportService';
import { FiChevronDown, FiChevronUp, FiDownload, FiEye, FiFileText, FiRotateCcw } from 'react-icons/fi';
import { type MultiValue } from "react-select";
import { coursesDropDownList } from '../../../services/subscriptionService';
import toast from 'react-hot-toast';
import GlassButton from '../../../components/Button/Button';
import {getStudyPlanReport } from '../../../store/slices/userReportSlice';
import { DateInput, Filter } from '../../../components/ReportComponent/reportFilter';
import ViewStudentStudyPlan from './ReportModals/ViewStduentStudyPlan';


const UserStudyPlanReport = () => {
    ...
    useEffect(() => {
        ...
        dispatch(getStudyPlanReport(filters));
    }, [
    ...
    useEffect(() => {
        const transformed = data.map((user, index) => ({
            ...
            id: user.id,
            user_id: user?.user,
            course_name: user?.course__name,
            subject_detail: user?.subject__subject__name,
            completed_studyplan_count: user?.completed_studyplan_count,
            uncompleted_studyplan_count: user?.uncompleted_studyplan_count,
            user__category: user?.user__category,
            user__first_name: user?.user__first_name,
            user__last_name: user?.user__last_name,
            user__email: user?.user__email,
            user__reference_id: user?.user__reference_id || 'N/A',
            user__student_type: user?.user__student_type || 'N/A',
            user__phone1: user?.user__phone1 || 'N/A',
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'user__first_name', title: 'First Name', align: 'center' },
        { key: 'user__last_name', title: 'Last Name', align: 'center' },
        { key: 'user__email', title: 'Email', align: 'center' },
        { key: 'user__phone1', title: 'Phone', align: 'center' },
        { key: 'user__student_type', title: 'Student Type', align: 'center' },
        { key: 'user__category', title: 'Category', align: 'center' },
        { key: 'user__reference_id', title: 'Reference ID', align: 'center' },
        { key: 'course_name', title: 'Course Name', align: 'center' },
        { key: 'subject_detail', title: 'Subject Name', align: 'center' },
        { key: 'completed_studyplan_count', title: 'completed studyplan count', align: 'center' },
        { key: 'uncompleted_studyplan_count', title: 'Incompleted studyplan count', align: 'center' },
        {
            key: 'actions',
            title: 'Actions',
            align: 'center',
            render: (_, row) => (
                <div className="flex space-x-2">
                    <GlassButton
                        onClick={() => {
                            showModal({
                                title: 'Student Study Plan Details',
                                content: <ViewStudentStudyPlan student={row.user_id} subject={row.subject_id} />,
                                type: 'success',
                                size: 'xxl',
                            });
                        }}
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserStudyPlanPdfApi(filters);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserStudyPlanExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="backdrop-blur-md bg-white/30 border border-white/20 rounded-2xl p-4 shadow-lg">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-gray-800">Student Study Plan Report</h2>
                    <button
...
export default UserStudyPlanReport;
