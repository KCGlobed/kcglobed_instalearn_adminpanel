...
import { getActiveSubscription } from '../../../store/slices/reportActiveSubscriptionSlice';
import { activeSubscriptionExcelApi, activeSubscriptionPdfApi } from '../../../services/ReportService';
...
const ActiveSubscriptionReport = () => {
    const dispatch = useAppDispatch();
    const { showModal } = useModal();
    const { data, loading, pagination } = useAppSelector((state) => state.activeSubscription);
    ...
    useEffect(() => {
        ...
        dispatch(getActiveSubscription(filters));
    }, [
    ...
    useEffect(() => {
        const transformed = data.map((user, index) => ({
            ...
            courses: user.ordered_courses.map((course: any) => course.name).join(', '),
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'total_amount', title: 'Total Amount', align: 'center' },
    ], [showModal]);
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await activeSubscriptionPdfApi(filters);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await activeSubscriptionExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
...
export default ActiveSubscriptionReport;
