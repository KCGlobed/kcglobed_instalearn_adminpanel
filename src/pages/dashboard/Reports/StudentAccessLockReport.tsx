...
import { unLockedDeviceById, UserAccessDateApi, UserAccessExcelApi, UserExamDateApi, UserExamDateExcelApi } from '../../../services/ReportService';
...
import { getUserAccess, unLockedDevice } from '../../../store/slices/reportUserAccessSlice';
import toast from 'react-hot-toast';

const StudentAccessLockReport = () => {
    const dispatch = useAppDispatch();
    const { showModal } = useModal();
    const { data, loading, pagination } = useAppSelector((state) => state.userAccessReport);
    ...
    useEffect(() => {
        ...
        dispatch(getUserAccess(filters));
    }, [
    ...
    useEffect(() => {
        ...
        const transformed = data.map((user, index) => ({
            ...
            device_1: user?.user_devices[0]?.device_id || 'N/A',
            device_2: user?.user_devices[1]?.device_id || 'N/A',
            device_3: user?.user_devices[2]?.device_id || 'N/A',
            is_locked: user.user_detail?.is_locked,
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'ip_address', title: 'IP Address', align: 'center' },
        { key: 'device_type', title: 'Current Device Type', align: 'center' },
        { key: 'created_at', title: 'Locked Date', align: 'center' },
        { key: 'device_1', title: 'Device 1 ', align: 'center' },
        { key: 'device_2', title: 'Device 2', align: 'center' },
        { key: 'device_3', title: 'Device 3', align: 'center' },
        {
            key: 'is_locked',
            title: 'Is Locked',
            align: 'center',
            render: (_, row) => (
                <label className="inline-flex relative items-center cursor-pointer">
                    <input
                        ...
                        onChange={() => handleToggleStatus(row.id, row.is_locked)}
                        className="sr-only peer"
                    />
                    <div className="w-11 h-6 bg-gray-300 rounded-full peer peer-checked:bg-green-500 relative after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:after:translate-x-full" />
                </label>
            ),
        },
        {
            ...
            render: (_, row) => (
                row.is_locked ?
                    <span className="px-3 py-1 bg-red-100 text-red-800 rounded-full text-sm font-semibold cursor-pointer" onClick={() => handleUnLockedUser(row.user_detail_id)}>Locked</span>
                    :
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserAccessDateApi(filters);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserAccessExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
    ...
    const handleUnLockedUser = async (id: any) => {
        console.log("Unlocking user with ID:", id);
        const payload = { status: true };
        await unLockedDeviceById(id, payload)
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="backdrop-blur-md bg-white/30 border border-white/20 rounded-2xl p-4 shadow-lg">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-gray-800">Student Access Lock Report</h2>
                    <button
...
export default StudentAccessLockReport;
