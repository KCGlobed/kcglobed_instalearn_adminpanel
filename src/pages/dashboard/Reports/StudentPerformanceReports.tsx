...
import {  UserPerFormanceExcelApi, UserPerFormancePdfApi } from '../../../services/ReportService';
...
import { getUserPerformance } from '../../../store/slices/studentPerformanceSlice';
import { formatSeconds } from '../../../hooks/formatSeconds';

const StudentPerformanceReports = () => {
    const dispatch = useAppDispatch();
    const { showModal } = useModal();
    const { data, loading, pagination } = useAppSelector((state) => state.userPerformance);
    ...
    useEffect(() => {
        ...
        dispatch(getUserPerformance(filters));
    }, [
    ...
    useEffect(() => {
        ...
        const transformed = data.map((user, index) => ({
            ...
            total_video_watched: user.performance_report?.total_video_watched ,
            watch_time:formatSeconds(user.performance_report?.watch_time ) || 'N/A',
            mcq_score: `${user.performance_report?.mcq_score }%`|| 'N/A',
            sim_score:`${user.performance_report?.sim_score }%` || 'N/A',
            latest_mock_score: `${user.performance_report?.latest_mock_score }%` || 'N/A',
            avg_mock_score: `${user.performance_report?.avg_mock_score }%` || 'N/A',
            real_time_score: `${user.performance_report?.real_time_score }%` || 'N/A',
            incompleted_sessions: user.performance_report?.incompleted_sessions ,
            completed_sessions: user.performance_report?.completed_sessions ,
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'total_video_watched', title: 'Total Video Watched', align: 'center' },
        { key: 'watch_time', title: 'Watch Time', align: 'center' },
        { key: 'mcq_score', title: 'MCQ Score', align: 'center' },
        { key: 'sim_score', title: 'SIM/Essay Score', align: 'center' },
        { key: 'latest_mock_score', title: 'Latest Mock Score', align: 'center' },
        { key: 'avg_mock_score', title: 'Average Mock Score', align: 'center' },
        { key: 'real_time_score', title: 'Real Time Score', align: 'center' },
        { key: 'incompleted_sessions', title: 'Paused Sessions', align: 'center' },
        { key: 'completed_sessions', title: 'Completed Sessions', align: 'center' },
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserPerFormancePdfApi(filters);
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserPerFormanceExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="backdrop-blur-md bg-white/30 border border-white/20 rounded-2xl p-4 shadow-lg">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-gray-800">Student Performance Report</h2>
                    <button
