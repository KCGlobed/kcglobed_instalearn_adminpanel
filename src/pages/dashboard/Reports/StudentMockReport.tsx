...
import { userMockIndividualPdfApi, UserMockReportExcelApi, UserMockReportPdfApi, userNoteIndividualPdfApi, UserNotesReportExcelApi, UserNotesReportPdfApi } from '../../../services/ReportService';
...
import ViewNotesReport from './ReportModals/ViewNotesReport';
import { getMockReport } from '../../../store/slices/userReportSlice';
import { DateInput, Filter } from '../../../components/ReportComponent/reportFilter';

const StudentMockReport = () => {
    ...
    useEffect(() => {
        ...
        dispatch(getMockReport(filters));
    }, [
    ...
    useEffect(() => {
        const transformed = data.map((user, index) => ({
            ...
            mock_test_count: user?.mock_test_count,
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'mock_test_count', title: 'Mock Test Count', align: 'center' },
        {
            key: 'actions',
            title: 'Actions',
            align: 'center',
                ...
                <div className="flex space-x-2">
                    {/* <GlassButton
                        onClick={() => {
                            showModal({
                                title: 'Edit Topic',
                                content: <ViewNotesReport student={row.user_id} subject={row.subject_id} />,
                                type: 'success',
                                size: 'md',
                            });
                        }}
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserMockReportPdfApi(filters);
    ...
    const handleIndividualReportPdf = async (data: any) => {
        try {
            const res = await userMockIndividualPdfApi(data.user_id, data.subject_id);;
            const fileUrl = res?.data?.report_url;
            if (fileUrl) window.open(fileUrl, "_blank");
        } finally {
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserMockReportExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
        ...
        <div className="p-6 space-y-5">
            ...
            <div className="backdrop-blur-md bg-white/30 border border-white/20 rounded-2xl p-4 shadow-lg">
                <div className="flex items-center justify-between">
                    <h2 className="text-2xl font-bold text-gray-800">Student Mock Test Report</h2>
                    <button
...
export default StudentMockReport;
