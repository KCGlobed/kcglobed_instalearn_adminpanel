...
import { UserAccessDateApi, UserAccessExcelApi, UserAssessmentExcelApi, UserAssessmentIndividualPdfApi, UserAssessmentPdfApi} from '../../../services/ReportService';
...
import { getUserAssessment } from '../../../store/slices/reportAssementSlice';
import toast from 'react-hot-toast';

const AssessmentReport = () => {
    const dispatch = useAppDispatch();
    const { showModal } = useModal();
    const { data, loading, pagination } = useAppSelector((state) => state.reportAssement);
    ...
    useEffect(() => {
        ...
        dispatch(getUserAssessment(filters));
    }, [
    ...
    useEffect(() => {
        const transformed = data.map((user, index) => ({
            ...
            course_Id: user?.course || 'N/A',
            subject_detail: user.subject_detail.name || 'N/A',
            subject_Id: user.subject|| 'N/A',
            score: `${user.score}%` || 'N/A',
            status: user.status ? 'Completed' : 'Incompleted',
            student_id: user.user_detail?.id || 'N/A',
    ...
    const columns: ColumnDefinition<any>[] = useMemo(() => [
        ...
        { key: 'status', title: 'Is Complete', align: 'center' },
        { key: 'subject_Id', title: 'Download', align: 'center',
            render: (_,row) => (
                <>
                 {
                  row?.status ==='Completed' ?
                     <>
                       <button className='mr-2 cursor-pointer' onClick={()=>handleIndividualReportPdf(row)}> <FiDownload /></button>
                     </>
                     : 'N/A'
                  }
    ...
    const handleDownloadpdf = async () => {
        ...
        try {
            setPdfLoading(true);
            const res = await UserAssessmentPdfApi(filters);
    ...
    const handleIndividualReportPdf = async (data:any) => {
        try {
            ...
            const res = await UserAssessmentIndividualPdfApi(data.student_id,data.subject_Id); ;
            const fileUrl = res?.data?.report_url;
            if (fileUrl) window.open(fileUrl, "_blank");
        } finally {
    ...
    const handleDownloadExcel = async () => {
        ...
        try {
            ...
            const res = await UserAssessmentExcelApi(filters);
            const fileUrl = res?.data?.report_url;
            if (fileUrl) {
