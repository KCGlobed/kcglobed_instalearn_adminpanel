...
export interface StudentRegistrationsChartProps {
  studentsGraph: any[];
  loading: boolean;
  filter: string;
  chartType: ChartType;
...
export const StudentRegistrationsChart: React.FC<StudentRegistrationsChartProps> = ({
  studentsGraph,
  loading,
  filter,
  chartType,
  ...
  const studentList = useMemo(
    () => sortGraphDataChronologically(Array.isArray(studentsGraph) ? studentsGraph : []),
    [studentsGraph]
  );
  const studentLabels = useMemo(
    () => studentList.map((item: any) => formatGraphLabel(item, filter)),
    [studentList, filter]
  );
  const studentCounts = useMemo(
    () => studentList.map((item: any) => Number(item.total_student_registered) || 0),
    [studentList]
  );
    ...
    datasets: [
      {
        ...
        borderColor:
          chartType === "doughnut"
            ...
            ? "#4f46e5"
            : "#ffffffff",
        hoverBackgroundColor: chartType === "doughnut" ? undefined : "rgba(67, 56, 202, 1)",
  ...
  }), [studentLabels, studentCounts, chartType]);
        ...
        callbacks: {
          label: (context: any) =>
            chartType === "doughnut"
              ...
              : ` Students Registered: ${context.parsed.y}`,
        },
      },
    },
  ...
  return (
    <div className="w-full bg-white rounded-2xl border border-gray-100 shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-5 flex flex-col h-[390px] transition-all hover:shadow-[0_6px_24px_rgba(79,70,229,0.06)]">
      ...
      <div className="flex-1 w-full min-h-0 relative">
        ...
        {(chartType === "doughnut" ? hasStudentData : studentList.length > 0) ? (
