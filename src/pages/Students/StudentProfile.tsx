...
const StudentProfile: React.FC = () => {
    ...
    const { active_orders, courses, user_devices } = selectedStudent;
            ...
            <div className="relative h-48 w-full bg-slate-100 flex-shrink-0">
                ...
                <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex items-end">
                    <div className="p-8 w-full flex flex-col sm:flex-row items-start sm:items-end gap-6 relative">
                        <button
                            onClick={() => navigate('/dashboard/students')}
                            className="absolute top-4 left-8 text-white/70 hover:text-white flex items-center gap-2 text-xs font-semibold transition-colors z-10"
                        >
                            <ArrowLeft size={14} /> Back
                        </button>
                        <div className="w-24 h-24 rounded-2xl border-4 border-white bg-white overflow-hidden shadow-2xl flex-shrink-0 mt-8">
                            {selectedStudent.image ? (
                                <img src={selectedStudent.image} alt="Avatar" className="w-full h-full object-cover" />
                            ) : (
                        ...
                        <div className="flex flex-wrap gap-6 border-b border-slate-100 pb-0">
                            {(['overview', 'courses', 'orders', 'devices'] as const).map((tab) => (
                                <button
                                    ...
                                    className={`text-[11px] font-bold uppercase tracking-[0.1em] pb-3 border-b-2 transition-colors ${
                                        activeTab === tab 
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-[13px] font-semibold text-slate-700">{selectedStudent.phone1 || 'Not provided'}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-3">
