...
import { getSupportSubTopic, removeSupportSubTopic, updateSupportSubTopicStatus } from '../../../store/slices/supportSubTopicSlice';
...
import { deleteSupportSubTopicApi } from '../../../services/apiServices';
import InlineDateFilter from '../../../components/common/InlineDateFilter';
import SortDropdown from '../../../components/common/SortDropdown';
import SearchInput from '../../../components/common/SearchInput';
import DynamicFilter from '../../../components/common/DynamicFilter';
import { supportSubTopicFilterConfig } from '../../../utils/filterConfiguration';
import SupportSubTopicForm from '../../../components/Forms/supportSubTopicForm';
...
const ManageSupportSubTopic: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state) => state.supportSubTopic);
    const pageSize = 5;
    ...
    const columns: ColumnDef[] = [
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex flex-col">
                    <span className="font-semibold text-gray-900 text-sm">{row.title}</span>
                    {row.slug && (
                        <span className="text-[11px] text-gray-500 font-medium">{row.slug}</span>
                    )}
                </div>
            ),
            sortable: true,
        ...
        },
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex items-center gap-2">
                    <div className="px-2.5 py-1 bg-indigo-50 border border-indigo-100 rounded-lg text-xs font-semibold text-indigo-600">
                        {row.main_topic?.title || 'None'}
                    </div>
                </div>
            ),
        ...
        },
        {
            ...
            render: (value: boolean, row: any) => (
                <button
                    onClick={() => {
                        dispatch(updateSupportSubTopicStatus({ id: row.id, status: !value }))
                            .unwrap()
                            .then(() => toast.success(`Support subtopic ${!value ? 'activated' : 'deactivated'} successfully`))
            ...
            render: (_, row) => (
                <div className="flex items-center justify-end gap-3 pr-2">
                    ...
                    <GlassButton
                        ...
                        onClick={() => {
                            showModal({
                                title: 'Delete Support Subtopic',
                                content: <DeleteConfirmationModal
                                    id={row}
                                    name={row.title}
                                    onDelete={async () => {
                                        await deleteSupportSubTopicApi(row.id);
                                        dispatch(removeSupportSubTopic(row.id));
                                    }}
                                />,
                                type: 'custom',
                                size: 'md',
    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                ...
                <DynamicFilter
                    show={showFilter}
                    config={supportSubTopicFilterConfig}
                    values={filters}
