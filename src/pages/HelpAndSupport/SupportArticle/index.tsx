...
import { getSupportArticle, removeSupportArticle, updateSupportArticleStatus } from '../../../store/slices/supportArticleSlice';
...
const ManageSupportArticle: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state) => state.supportArticle);
    const pageSize = 10;
    ...
    const columns: ColumnDef[] = [
        ...
        },
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex flex-col gap-1">
                    <span className="text-xs font-medium text-indigo-700 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 inline-block w-max">
                        {row.main_topic?.title || 'N/A'}
                    </span>
                    <span className="text-xs font-medium text-emerald-700 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-100 inline-block w-max">
                        {row.sub_topic?.title || 'N/A'}
                    </span>
                </div>
            ),
            width: '200px',
        ...
        },
        {
            ...
            render: (value: boolean, row: any) => (
                <button
                    onClick={() => {
                        dispatch(updateSupportArticleStatus({ id: row.id, status: !value }))
                            .unwrap()
                            .then(() => toast.success(`Support Article ${!value ? 'activated' : 'deactivated'} successfully`))
                        ...
                        onClick={() => {
                            showModal({
                                ...
                                content: <DeleteConfirmationModal
                                    ...
                                    name={row.title}
                                    onDelete={async () => {
                                        await deleteSupportArticleApi(row.id);
                                        dispatch(removeSupportArticle(row.id));
                                    }}
                                />,
                                type: 'custom',
                                size: 'md',
    ...
    return (
        <div className="flex flex-col gap-6 animate-in fade-in duration-500">
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                ...
                <DynamicFilter
                    show={showFilter}
                    config={supportArticleFilterConfig}
                    values={filters}
