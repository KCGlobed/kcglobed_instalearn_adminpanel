...
import { getQuiz, deleteQuiz, updateQuizStatus } from '../../store/slices/QuizSlice';
...
import AssignMcqForm from '../../components/Forms/AssignMcqForm';
import GlassButton from '../../components/Button/Button';
import { FiEdit, FiTrash, FiEye, FiSettings } from 'react-icons/fi';
import DeleteConfirmationModal from '../../components/Modal/DeleteModal';
import InlineDateFilter from '../../components/common/InlineDateFilter';
import SortDropdown from '../../components/common/SortDropdown';
import SearchInput from '../../components/common/SearchInput';
import DynamicFilter from '../../components/common/DynamicFilter';
import { quizFilterConfig as filterConfig } from '../../utils/filterConfiguration';
...
const ManageQuiz: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state) => state.quiz);
    const pageSize = 5;
    ...
    const columns: ColumnDef[] = [
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex items-center gap-3">
                    {row.thumbnail ? (
                        <img src={row.thumbnail} alt={row.name} className="w-10 h-10 rounded-lg object-cover bg-gray-50 border border-gray-100 shadow-sm" />
                    ) : (
                        <div
                            className="w-10 h-10 rounded-lg flex items-center justify-center font-bold text-sm shadow-sm bg-indigo-50 text-indigo-500 border border-indigo-100"
                        >
                    ...
                    <div className="flex flex-col">
                        ...
                        {row.chapter && row.chapter.name && (
                            <span className="text-[11px] text-gray-500 font-medium whitespace-nowrap">Chapter: {row.chapter.name}</span>
                        )}
                    </div>
                </div>
            ),
        ...
        },
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex flex-col">
                    <span className="text-gray-800 text-xs font-semibold">{row.total_question} Questions</span>
                    <span className="text-gray-500 text-[11px] font-medium">Pass: {row.pass_percentage}%</span>
                </div>
            ),
            width: '120px',
        },
        ...
        },
        {
            ...
            render: (value: boolean, row: any) => (
                <button
                    onClick={async () => {
                        try {
                            await dispatch(updateQuizStatus({ id: row.id, status: !row.status })).unwrap();
                ...
                <div className="flex items-center justify-end gap-3 pr-2">
                    ...
                    <GlassButton
                        ...
                        onClick={() =>
                            showModal({
                                title: 'Edit Quiz',
                                content: <QuizForm quizId={row.id} />,
                                type: 'custom',
                                size: 'lg',
                            })
                        }
                                ...
                                content: <DeleteConfirmationModal
                                    ...
                                    onDelete={async (id) => {
                                        try {
                                            ...
                                            toast.error(err?.message || "Failed to remove quiz");
                                        }
                                    }}
                                />,
                                type: 'custom',
