...
import { getStudentPerformance } from '../../../store/slices/studentPerformanceSlice';
...
import { downloadStudentPerformancePdfApi, downloadStudentPerformanceExcelApi } from '../../../services/apiServices';
import { studentPerformanceFilterConfig } from '../../../utils/filterConfiguration';
import StudentPerformanceView from '../../../components/View/StudentPerformanceView';
...
const StudentPerformace: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state) => state.studentPerformance);
    const pageSize = 10;
    ...
    const columns: ColumnDef[] = [
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex items-center gap-3">
                    ...
                    >
                        <span>{row.user_detail?.first_name ? row.user_detail.first_name.charAt(0).toUpperCase() : 'S'}</span>
                    </div>
                    <div className="flex flex-col">
                        <span className="font-semibold text-gray-900 text-sm whitespace-nowrap">
                            {row.user_detail?.first_name} {row.user_detail?.last_name}
                        </span>
                        <span className="text-gray-400 text-[10px]">ID: {row.user_detail?.id}</span>
                    </div>
                </div>
            ),
            sortable: true,
        ...
        },
        {
            ...
            title: 'Email',
            render: (_: any, row: any) => (
                <span className="text-gray-800 text-sm">{row.user_detail?.email || '-'}</span>
            ),
            sortable: true,
            width: '200px',
        },
        {
            ...
            title: 'Phone',
            render: (_: any, row: any) => (
                <span className="text-gray-800 text-sm">{row.user_detail?.phone1 || '-'}</span>
            ),
            width: '150px',
        },
        {
            ...
            title: 'Course',
            render: (_: any, row: any) => (
                <span className="text-gray-800 text-sm font-semibold">{row.course_detail?.name || '-'}</span>
            ),
            sortable: true,
            width: '200px',
        },

        {
            ...
            title: 'Category',
            render: (_: any, row: any) => (
                <span className="text-gray-800 text-sm">{row.user_detail?.category || '-'}</span>
            ),
            width: '120px',
        },
        {
            ...
            render: (_: any, row: any) => (
                <div className="flex flex-col">
                    <span className="text-gray-800 text-sm font-semibold">{row.user_detail?.date_joined ? moment(row.user_detail.date_joined).format('MMM DD, YYYY') : '-'}</span>
                </div>
            ),
            sortable: true,
            width: '150px',
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                <div className="flex flex-wrap items-center justify-between px-5 py-4 gap-4">
                    ...
                    <SearchInput
                        ...
                        placeholder="Search student performance..."
                        className="mx-4"
                    />

                    <div className="flex items-center gap-4">
                        <ExportFile
                            pdfApi={() => downloadStudentPerformancePdfApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                email: debouncedFilters.email,
                            ...
                            excelApi={() => downloadStudentPerformanceExcelApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                email: debouncedFilters.email,
                            ...
                            fileNamePrefix="student-performance"
                        />
                    </div>
                </div>
