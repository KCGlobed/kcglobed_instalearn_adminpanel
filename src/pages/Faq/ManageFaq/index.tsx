...
import { getFaq, removeFaq, updateFaqStatus } from '../../../store/slices/faqSlice';
  ...
  const columns: ColumnDef[] = [
      ...
      render: (_: any, row: any) => (
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            ...
            {row.faq_topic && row.faq_topic.title && (
              <span className="text-[11px] text-gray-500 font-medium whitespace-nowrap">Sub of {row.faq_topic.title}</span>
            )}
          </div>
        </div>
      ),
    ...
    },
    {
      ...
      render: (value: boolean, row: any) => (
        <button
          onClick={() => {
            dispatch(updateFaqStatus({ id: row.id, status: !value }))
              .unwrap()
              .then(() => toast.success(`FAQ ${!value ? 'activated' : 'deactivated'} successfully`))
        ...
        <div className="flex items-center justify-end gap-3 pr-2">
          <GlassButton
            ...
            onClick={() =>
              showModal({
                title: 'Edit FAQ',
                content: <ManageFaqForm faqData={row} />,
                type: 'success',
                size: 'md',
              })
            }
