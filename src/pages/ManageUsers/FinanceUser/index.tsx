...
import { getFinanceUsers, updateFinanceUserStatus } from '../../../store/slices/financeUserSlice';
...
import { financeUserFilterConfig } from '../../../utils/filterConfiguration';
import InlineDateFilter from '../../../components/common/InlineDateFilter';
import DynamicServerTable from '../../../components/Table/Table';
import FinanceUserForm from '../../../components/Forms/FinanceUserForm';
import ExportFile from '../../../components/Forms/ExportFile';
import { downloadFinanceUserPdfApi, downloadFinanceUserExcelApi } from '../../../services/apiServices';
...
const ManageFinanceUsers: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state: any) => state.financeUser);
    const pageSize = 10;
            ...
            render: (_, row: any) => {
                ...
                return (
                    <button
                        onClick={async () => {
                            try {
                                await dispatch(updateFinanceUserStatus({ id: row.id, status: !isActive })).unwrap();
                                toast.success(`Finance User status updated`);
                            } catch (err) {
            ...
            render: (_, row) => (
                <div className="flex items-center justify-end gap-3 pr-2">
                    <GlassButton
                        ...
                        onClick={() =>
                            showModal({
                                title: 'Edit Finance User Profile',
                                content: <FinanceUserForm FinanceUserData={row} />,
                                type: 'custom',
                                size: 'xxl',
                            })
                        }
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                <div className="flex flex-wrap items-center justify-between px-5 py-4 gap-4">
                    ...
                    <SearchInput
                        ...
                        placeholder="Search Finance Users..."
                        className="mx-4"
                    />

                    <div className="flex items-center gap-4">
                        <ExportFile
                            pdfApi={() => downloadFinanceUserPdfApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                            ...
                            excelApi={() => downloadFinanceUserExcelApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                email: debouncedFilters.email,
                            ...
                            fileNamePrefix="finance_users"
                        />
                        <button
                            onClick={() => showModal({
                                title: 'Create New Finance User',
                ...
                <DynamicFilter
                    show={showFilter}
                    config={financeUserFilterConfig}
                    values={filters}
...
export default ManageFinanceUsers;
