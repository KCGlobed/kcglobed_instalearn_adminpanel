...
import { getContentManagementUsers, updateContentManagementUserStatus } from '../../../store/slices/contentManagementSlice';
...
import { contentManagementUserFilterConfig } from '../../../utils/filterConfiguration';
import InlineDateFilter from '../../../components/common/InlineDateFilter';
import DynamicServerTable from '../../../components/Table/Table';
import ContentManagementUserForm from '../../../components/Forms/ContentManagementUserForm';
import ExportFile from '../../../components/Forms/ExportFile';
import { downloadContentManagementUserPdfApi, downloadContentManagementUserExcelApi } from '../../../services/apiServices';
...
const ManageContentManagementUsers: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state: any) => state.contentManagementUser);
    const pageSize = 10;
    ...
    useEffect(() => {
        dispatch(getContentManagementUsers({
            page: currentPage,
            ...
            render: (_, row: any) => {
                ...
                return (
                    <button
                        onClick={async () => {
                            try {
                                await dispatch(updateContentManagementUserStatus({ id: row.id, status: !isActive })).unwrap();
                                toast.success(`Content Management User status updated`);
                            } catch (err) {
            ...
            render: (_, row) => (
                <div className="flex items-center justify-end gap-3 pr-2">
                    <GlassButton
                        ...
                        onClick={() =>
                            showModal({
                                title: 'Edit Content Management User Profile',
                                content: <ContentManagementUserForm ContentManagementUserData={row} />,
                                type: 'custom',
                                size: 'xxl',
                            })
                        }
            ...
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                <div className="flex flex-wrap items-center justify-between px-5 py-4 gap-4">
                    ...
                    <SearchInput
                        ...
                        placeholder="Search Content Management Users..."
                        className="mx-4"
                    />

                    <div className="flex items-center gap-4">
                        <ExportFile
                            pdfApi={() => downloadContentManagementUserPdfApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                            ...
                            excelApi={() => downloadContentManagementUserExcelApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                email: debouncedFilters.email,
                            ...
                            fileNamePrefix="content_management_users"
                        />
                        <button
                            onClick={() => showModal({
                                title: 'Create New Content Management User',
                                content: <ContentManagementUserForm />,
                                type: 'custom',
                                size: 'xxl'
                            })}
                ...
                <DynamicFilter
                    show={showFilter}
                    config={contentManagementUserFilterConfig}
                    values={filters}
...
export default ManageContentManagementUsers;
