...
import { getSalesUsers, updateSalesUserStatus } from '../../../store/slices/salesUserSlice';
...
import { salesUserFilterConfig } from '../../../utils/filterConfiguration';
import InlineDateFilter from '../../../components/common/InlineDateFilter';
import DynamicServerTable from '../../../components/Table/Table';
import SalesUserForm from '../../../components/Forms/SalesUserForm';
import ExportFile from '../../../components/Forms/ExportFile';
import { downloadSalesUserPdfApi, downloadSalesUserExcelApi } from '../../../services/apiServices';
...
const ManageSalesUsers: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state: any) => state.salesUsers);
    const pageSize = 10;
            ...
            render: (_, row: any) => {
                ...
                return (
                    <button
                        onClick={async () => {
                            try {
                                await dispatch(updateSalesUserStatus({ id: row.id, status: !isActive })).unwrap();
                                toast.success(`Sales User status updated`);
                            } catch (err) {
            ...
            render: (_, row) => (
                <div className="flex items-center justify-end gap-3 pr-2">
                    <GlassButton
                        ...
                        onClick={() =>
                            showModal({
                                title: 'Edit Sales User Profile',
                                content: <SalesUserForm SalesUserData={row} />,
                                type: 'custom',
                                size: 'xl',
                            })
                        }
        ...
        <div className="flex flex-col gap-6 w-full animate-in fade-in duration-500">
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                <div className="flex flex-wrap items-center justify-between px-5 py-4 gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <ExportFile
                            pdfApi={() => downloadSalesUserPdfApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                            ...
                            excelApi={() => downloadSalesUserExcelApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                email: debouncedFilters.email,
                            ...
                            fileNamePrefix="sales_users"
                        />
                        <button
                            onClick={() => showModal({
                                title: 'Create New Sales User',
                                content: <SalesUserForm />,
                                type: 'custom',
                                size: 'xl'
                            })}
...
export default ManageSalesUsers;
