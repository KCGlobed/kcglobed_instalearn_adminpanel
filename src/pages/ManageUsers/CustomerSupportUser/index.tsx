...
import { getCustomerSupportUsers, updateCustomerSupportUserStatus } from '../../../store/slices/customerSupportUserSlice';
...
import { customerSupportUserFilterConfig } from '../../../utils/filterConfiguration';
import InlineDateFilter from '../../../components/common/InlineDateFilter';
import DynamicServerTable from '../../../components/Table/Table';
import CustomerSupportUserForm from '../../../components/Forms/CustomerSupportUserForm';
import ExportFile from '../../../components/Forms/ExportFile';
import { downloadCustomerSupportUserPdfApi, downloadCustomerSupportUserExcelApi } from '../../../services/apiServices';
...
const ManageCustomerSupportUsers: React.FC = () => {
    ...
    const { data, loading, pagination } = useAppSelector((state: any) => state.customerSupportUser);
    const pageSize = 10;
    ...
    useEffect(() => {
        dispatch(getCustomerSupportUsers({
            page: currentPage,
            ...
            render: (_, row: any) => {
                ...
                return (
                    <button
                        onClick={async () => {
                            try {
                                await dispatch(updateCustomerSupportUserStatus({ id: row.id, status: !isActive })).unwrap();
                    ...
                    <GlassButton
                        ...
                        onClick={() =>
                            showModal({
                                ...
                                content: <CustomerSupportUserForm CustomerSupportUserData={row} />,
                                type: 'custom',
                                size: 'xxl',
                            })
                        }
        ...
        <div className="flex flex-col gap-6 w-full animate-in fade-in duration-500">
            <div className="flex flex-col bg-white rounded-2xl shadow-[0_2px_15px_-3px_rgba(0,0,0,0.07),0_10px_20px_-2px_rgba(0,0,0,0.04)] border border-gray-100 relative">
                <div className="flex flex-wrap items-center justify-between px-5 py-4 gap-4">
                    ...
                    <div className="flex items-center gap-4">
                        <ExportFile
                            pdfApi={() => downloadCustomerSupportUserPdfApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                            ...
                            excelApi={() => downloadCustomerSupportUserExcelApi({
                                search: debouncedSearchTerm,
                                first_name: debouncedFilters.first_name,
                                last_name: debouncedFilters.last_name,
                                status: debouncedFilters.is_active,
                            ...
                            fileNamePrefix="customer_support_users"
                        />
                        <button
                            onClick={() => showModal({
                                title: 'Create New Marketing User',
                                content: <CustomerSupportUserForm />,
                                type: 'custom',
                                size: 'xxl'
                            })}
                ...
                <DynamicFilter
                    show={showFilter}
                    config={customerSupportUserFilterConfig}
                    values={filters}
...
export default ManageCustomerSupportUsers;
